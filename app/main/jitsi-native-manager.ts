...
import { 
    getSourcePickerCode, 
    injectSourcePickerFunction, 
    logSourcePickerEvent,
    getSimplifiedScreenShareInterceptorCode  // НОВЫЙ ИМПОРТ
} from "./electron-custom-source-picker";
...
export class JitsiNativeManager {
    ...
    private async injectHandlers(): Promise<void> {
        ...
        try {
            ...
            await this.state.window.webContents.executeJavaScript(
                getSimplifiedScreenShareInterceptorCode()
            );
            ...
            const result = await this.state.window.webContents.executeJavaScript(`
                (async function() {
                    ...
                    try {
                        ...
                        window.audioRoutingMode = null;
                        window.participantAudioMixer = null;
                        window.privateAudioProcessors = null;
